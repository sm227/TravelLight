...
import org.example.travellight.entity.Entity;  // Entity 엔티티 클래스 import
...
public class EntityTestRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        // Entity 객체 생성 (destination 필드만 설정)
        Entity entity = new Entity();
        entity.setDestination("Paris");  // destination 필드만 설정

        // TravelService를 통해 데이터베이스에 저장
        Entity savedEntity = travelService.saveTravel(entity); // TravelService의 메서드 호출
        System.out.println("저장된 여행: " + savedEntity);
    }
}
