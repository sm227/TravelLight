...
public class UserController {
    ...
    public ResponseEntity<CommonApiResponse<UserDto.UserResponse>> register(
            @Parameter(description = "회원가입 정보", required = true)
            @RequestBody UserDto.RegisterRequest request) {
        UserDto.UserResponse response = userService.register(request);
        return ResponseEntity.ok(CommonApiResponse.success("회원가입이 완료되었습니다.", response));
    }
    ...
    @PostMapping("/login")
    public ResponseEntity<CommonApiResponse<UserDto.UserResponse>> login(
            @Parameter(description = "로그인 정보", required = true)
            @RequestBody UserDto.LoginRequest request) {
        UserDto.UserResponse response = userService.login(request);
        return ResponseEntity.ok(CommonApiResponse.success("로그인이 완료되었습니다.", response));
    }
