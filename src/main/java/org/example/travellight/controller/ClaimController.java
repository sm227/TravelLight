...
public class ClaimController {
    ...
    @PostMapping
    public ResponseEntity<CommonApiResponse<ClaimDto.ClaimResponse>> createClaim(
        ...
        return ResponseEntity.ok(CommonApiResponse.success("클레임이 성공적으로 생성되었습니다.", response));
    }
    ...
    public ResponseEntity<CommonApiResponse<List<ClaimDto.ClaimResponse>>> getClaimsByUserId(
            @Parameter(description = "사용자 ID", required = true)
            @PathVariable Long userId) {

        List<ClaimDto.ClaimResponse> response = claimService.getClaimsByUserId(userId);
        return ResponseEntity.ok(CommonApiResponse.success("클레임 목록을 성공적으로 조회했습니다.", response));
    }
    ...
    @GetMapping
    public ResponseEntity<CommonApiResponse<List<ClaimDto.ClaimResponse>>> getAllClaims() {
    ...
    @GetMapping("/{claimId}")
    public ResponseEntity<CommonApiResponse<ClaimDto.ClaimResponse>> getClaimById(
            @Parameter(description = "클레임 ID", required = true)
            @PathVariable Long claimId) {

        ClaimDto.ClaimResponse response = claimService.getClaimById(claimId);
        return ResponseEntity.ok(CommonApiResponse.success("클레임 상세 정보를 성공적으로 조회했습니다.", response));
    }
    ...
    @PutMapping("/{claimId}")
    public ResponseEntity<CommonApiResponse<ClaimDto.ClaimResponse>> updateClaim(
            @Parameter(description = "클레임 ID", required = true)
            @PathVariable Long claimId,
        ...
        return ResponseEntity.ok(CommonApiResponse.success("클레임이 성공적으로 수정되었습니다.", response));
    }
    ...
    public ResponseEntity<CommonApiResponse<Void>> deleteClaim(
            @Parameter(description = "클레임 ID", required = true)
            @PathVariable Long claimId) {

        claimService.deleteClaim(claimId);
        return ResponseEntity.ok(CommonApiResponse.success("클레임이 성공적으로 삭제되었습니다.", null));
    }
    ...
    public ResponseEntity<CommonApiResponse<ClaimDto.ClaimResponse>> resolveClaim(
            @Parameter(description = "클레임 ID", required = true)
            @PathVariable Long claimId,
        ...
        return ResponseEntity.ok(CommonApiResponse.success("클레임이 성공적으로 해결되었습니다.", response));
    }
