...
public class ReservationController {
    ...
    public ResponseEntity<ApiResponse<Boolean>> canWriteReview(
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse.error("리뷰 작성 가능 여부 확인 중 오류가 발생했습니다."));
        }
    }
    ...
    public ResponseEntity<ApiResponse<ReviewDto.ReviewResponse>> createReview(
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse.error("리뷰 작성 중 오류가 발생했습니다: " + e.getMessage()));
        }
    }
    
    // 예약의 리뷰 작성 상태 확인 API
    @GetMapping("/{reservationId}/review-status")
    public ResponseEntity<ApiResponse<Map<String, Object>>> getReviewStatus(
            @PathVariable Long reservationId) {
        ...
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success("리뷰 상태를 확인했습니다.", result));
        } catch (Exception e) {
            logger.error("리뷰 상태 확인 중 오류 발생: {}", e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error("리뷰 상태 확인 중 오류가 발생했습니다."));
        }
    }
    
    // 예약의 리뷰 조회 API
    @GetMapping("/{reservationId}/review")
    public ResponseEntity<ApiResponse<ReviewDto.ReviewResponse>> getReviewByReservation(
            @PathVariable Long reservationId) {
        ...
        try {
            ...
            if (review != null) {
                ...
                return ResponseEntity.ok(ApiResponse.success("리뷰를 조회했습니다.", review));
            } else {
                logger.info("조회할 리뷰 없음 - 예약 ID: {}", reservationId);
                return ResponseEntity.ok(ApiResponse.success("리뷰가 없습니다.", null));
            }
        } catch (Exception e) {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("리뷰 조회 중 오류가 발생했습니다: " + e.getMessage()));
        }
    }
    
    // 예약의 리뷰 수정 API
    @PutMapping("/{reservationId}/review")
    public ResponseEntity<ApiResponse<ReviewDto.ReviewResponse>> updateReviewByReservation(
            @PathVariable Long reservationId,
        ...
        try {
            ...
            if (existingReview == null) {
                ...
                return ResponseEntity.badRequest().body(ApiResponse.error("수정할 리뷰가 없습니다."));
            }
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("리뷰 수정 중 오류가 발생했습니다: " + e.getMessage()));
        }
    }
    
    // 테스트용: 예약의 리뷰 삭제 (관리자 또는 개발자용)
    @DeleteMapping("/{reservationId}/review")
    public ResponseEntity<ApiResponse<String>> deleteReviewByReservation(
            @PathVariable Long reservationId) {
        ...
        try {
            ...
            if (deleted) {
                ...
                return ResponseEntity.ok(ApiResponse.success("리뷰가 성공적으로 삭제되었습니다.", "deleted"));
            } else {
                logger.info("삭제할 리뷰 없음 - 예약 ID: {}", reservationId);
                return ResponseEntity.ok(ApiResponse.success("삭제할 리뷰가 없습니다.", "not_found"));
            }
        } catch (Exception e) {
